...
def get_pet_labels(image_dir):
    ...
    for list in in_files:
        #to ignore the file which starts with '.' to avoid bugs.
        if list.startswith('.')!= True:
            list = list.replace(".jpg","")
            list = list.lower().translate(translation_table).replace("_"," ")
            list = list.rstrip()
            pet_labels.extend([list])
